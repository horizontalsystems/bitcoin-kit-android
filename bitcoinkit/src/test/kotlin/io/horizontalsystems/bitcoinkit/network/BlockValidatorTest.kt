...
class BlockValidatorTest {
    ...
    fun validateHeader() {
        try {
            validator.validateHeader(Fixtures.block2, Fixtures.block1)
    ...
    fun validateHeader_invalidPOW() {
        ...
        block2.bits = CompactBits.encode(BigInteger(block2.headerHashReversedHex, 16).minus(BigInteger.valueOf(1L)))
    ...
    fun validateHeader_invalidHeader() {
        ...
        assertThrows<BlockValidatorException.WrongPreviousHeader> {
            validator.validateHeader(block2, block1)
        }
    }
    ...
    fun validateBits() {
        try {
            validator.validateBits(Fixtures.block2, Fixtures.block1)
        } catch (e: BlockValidatorException) {
