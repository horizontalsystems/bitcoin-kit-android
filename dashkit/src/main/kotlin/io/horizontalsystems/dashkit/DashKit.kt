...
class DashKit : AbstractKit, IInstantTransactionDelegate, BitcoinCore.Listener, InstantLockListener {
    ...
    private val instantLock: InstantLock
    ...
    private constructor(
        ...
        this.instantLock = InstantLock(instantTransactionManager)
...
//        }
        inserted.mapNotNull { it as? DashTransactionInfo }.forEach {
            instantLock.handle(it)
        }
        updated.mapNotNull { it as? DashTransactionInfo }.forEach {
            instantLock.handle(it)
        }
...
//        }
    ...
    override fun onUpdateInstantLock(transactionHash: ByteArray) {
        val transaction = dashStorage.getFullTransactionInfo(transactionHash) ?: return
        val transactionInfo = dashTransactionInfoConverter.transactionInfo(transaction)
        bitcoinCore.listenerExecutor.execute {
            listener?.onTransactionsUpdate(listOf(), listOf(transactionInfo))
